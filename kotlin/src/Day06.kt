fun main() {
    ...
    fun createGuard(pos: Position, dir: Position): Guard =
        Guard(listOf(), pos, dir)

    // find the number of visited fields
    fun List<String>.part1(): Int =
        // the guard keeps walking until it reaches the end of the map
        generateSequence(createGuard(Position(indexOfFirst { it.contains('^') }, first { it.contains('^') }.indexOf('^')), Position(-1, 0))) { guard ->
